  ...
  return (
    ...
    >
      <div className="flex gap-3 items-start">
        ...
        <div>
          <h2
            className={`font-semibold ${task.completed ? "line-through text-zinc-400" : ""}`}
          >
            {task.title}
          </h2>
          {task.description && (
            <p
              className={`text-sm mt-1 ${task.completed ? "line-through text-zinc-400" : "text-zinc-600 dark:text-zinc-300"}`}
            >
              {task.description}
            </p>
          )}
