...
The logout workflow securely terminates a user session by revoking the refresh token and optionally clearing all active sessions. The access token is added to a denylist until its natural expiry to prevent any further use.
...
sequenceDiagram
    ...
    alt User Confirms
        ...
        alt No Refresh Token
            API-->>App: 200 (no-op, already logged out)
        end
        ...
        Note over Auth: Add access token to denylist
        Auth->>Redis: SET denylist:{accessTokenJti} -> true (TTL: remaining token lifetime)
        
        API-->>App: 200 OK
        Note over API: Clear refresh_token cookie (Set-Cookie: max-age=0)
        
        App->>App: Clear accessToken from memory (Zustand)
        App->>App: Clear TanStack Query cache
        App->>App: Reset all Zustand stores
...
sequenceDiagram
    ...
    Client->>App: Click "Logout All Devices"
    App->>App: Show warning dialog
    
    alt User Confirms
        App->>API: POST /api/v1/auth/logout/all
        Note over Client,API: Authorization: Bearer accessToken
        
        API->>Auth: verifyAccessToken(token)
        Auth->>Auth: Extract userId from JWT
        
        Auth->>DB: DELETE FROM refresh_tokens WHERE user_id = :userId
        Auth->>Redis: DEL user_sessions:{userId}
        Note over Redis: Clears all session data
        
        Auth->>Redis: SET denylist:{accessTokenJti} -> true (TTL: remaining)
        
        API-->>App: 200 OK
        Note over API: Clear refresh_token cookie
        
        App->>App: Clear all local state
...
export class AuthController {
  @Post('logout')
  @UseGuards(AuthGuard) // Optional - can logout without valid access token
  async logout(
    @Req() req: Request,
    @Res({ passthrough: true }) res: Response,
  ) {
    const refreshToken = this.extractRefreshTokenCookie(req);
...
|---|---|---|---|
| 200 | SUCCESS | Logout successful | Clear local state, redirect |
| 401 | UNAUTHORIZED | Invalid access token (logout/all) | Still clear local state |
| 429 | RATE_LIMITED | Too many logout requests | Still clear local state |

## Security Considerations

1. **Logout should always succeed from the client's perspective**: Even if the API call fails, the client should clear local state.
2. **Access token denylist**: The current access token is added to a Redis denylist to prevent its use even before natural expiry.
3. **Cookie clearing**: The refresh token cookie is cleared server-side via `Set-Cookie: max-age=0`.
4. **All-devices logout**: Revokes all refresh tokens from the database, forcing re-authentication on all devices.
5. **Audit logging**: All logout events are logged with user ID, timestamp, and IP address.
</content>
</｜DSML｜>

<write_to_file>
<path>docs/database/auth-schema.md</path>
<content># Authentication Database Schema
